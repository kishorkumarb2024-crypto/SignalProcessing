...
def plot(mag_db):
    plt.figure(figsize=(12, 6)) # sizing the image so that the dimensions of the plot-image is 12x6
...
def stft(audio):
    """
    The objective of computational STFT is to divide a longer time signal into shorter segments of equal length and then
    compute the Fourier transform separately on each shorter segment
    """
    data = librosa.stft(audio, n_fft=2048, hop_length=512, win_length=2048)
    mag, phase = librosa.magphase(data)
